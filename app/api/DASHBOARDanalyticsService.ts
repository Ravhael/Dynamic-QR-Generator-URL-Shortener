...
export interface AnalyticsSummary {
  ...
  activeShortURLs?: number; // Add this field
  effectiveDays?: number; // actual window used for charts
  scansByDay: {
    date: string;
    count: number;
  }[];
...
class AnalyticsService {
  ...
  public async getSummary(params: AnalyticsParams = {}): Promise<AnalyticsSummary> {
    ...
    query.append('days', (params.days ?? 30).toString());
    // Optional client-side debug passthrough for quick diagnostics
    if (typeof process !== 'undefined' && process.env.NEXT_PUBLIC_ANALYTICS_DEBUG === '1') {
      query.append('debug', '1');
    }
    const url = `/dashboardanalytics/summary?${query.toString()}`;
  ...
  public async getDetailedScans(params: AnalyticsParams = {}): Promise<DetailedScanEvent[]> {
    ...
    const url = `/dashboardanalytics/detailed-scans?${query.toString()}`;
    return apiClient.get(url);
  }
  ...
  public async getDetailedClicks(params: AnalyticsParams = {}): Promise<DetailedClickEvent[]> {
    ...
    if (params.limit) query.append('limit', params.limit.toString());
    const url = `/dashboardanalytics/detailed-clicks?${query.toString()}`;
    return apiClient.get(url);
  }
...
const dashboardanalyticsService = new AnalyticsService();
