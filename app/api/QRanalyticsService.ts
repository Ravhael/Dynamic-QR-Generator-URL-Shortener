...
export interface DetailedScanEvent {
  ...
  ipAddress: string; // IP address scanner
  location: {
    country: string;
    city: string;
    region?: string;
...
export interface DetailedClickEvent {
  ...
  shortUrl: string;
  clickedAt: string;
  ipAddress: string; // IP address pengclick
  location: {
    country: string;
    city: string;
    region?: string;
...
class QRAnalyticsService {
  ...
  public async getSummary(params: AnalyticsParams = {}): Promise<AnalyticsSummary> {
    ...
    const url = `/qr-analytics/summary?${query.toString()}`;
    console.warn('🔍 [QRanalyticsService] Final URL:', url);
    console.warn('🔍 [QRanalyticsService] ApiClient baseURL:', (apiClient as any).baseURL);
  ...
  public async getDetailedScans(params: AnalyticsParams = {}): Promise<DetailedScanEvent[]> {
    ...
    const url = `/qr-analytics/detailed-scans?${query.toString()}`;
    return apiClient.get(url);
  }
  ...
  public async getDetailedClicks(params: AnalyticsParams = {}): Promise<DetailedClickEvent[]> {
    ...
    if (params.limit) query.append('limit', params.limit.toString());
    const url = `/qr-analytics/detailed-clicks?${query.toString()}`;
    return apiClient.get(url);
  }
  ...
  public async getRecentActivity(limit: number = 10): Promise<{ activity: Array<DetailedScanEvent | DetailedClickEvent> }> {
    const url = `/qr-analytics/activity?limit=${limit}`;
    return apiClient.get(url);
  }
}
